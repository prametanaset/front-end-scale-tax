<template>
  <div class="w-full max-w-md mx-auto p-4">
    <!-- แก้ heading-level: ใส่ h1 เป็นหัวเรื่องแรกของเพจ (ซ่อนไว้ก็ได้) -->
    <p class="sr-only">รายการเอกสาร</p>
    ...
    <Accordion type="single" class="w-full space-y-4" collapsible>
      ...
      >
        <AccordionTrigger
          ...
          class="shadow-md px-2 flex items-center justify-between hover:no-underline py-2 cursor-pointer"
        >
          <!-- ซ้าย: ใช้ span (phrasing) แทน div -->
          <div class="inline-flex flex-col text-left gap-1">
            <h1 class="font-medium">{{ item.firstName }} {{ item.lastName }}</h1>
            <h2 class="text-sm text-gray-500">{{ item.content }}</h2>
          </div>

          <!-- ขวา: ใช้ span ทั้งหมด รวมถึง badge -->
          <template #icon>
            <span class="inline-flex flex-col text-right items-end gap-1">
              <span>฿{{ formatBaht(item.amount) }}</span>
              <!-- ถ้า Badge ของคุณเรนเดอร์เป็น div ให้เปลี่ยนมาใช้ span แทน -->
              <span
    ...
    <DrawerContent>
        ...
        <DrawerDescription class="flex flex-col pt-14">
          <BaseCard class="bg-gray-100/20">
            <CardHeader class="flex flex-col items-center relative gap-1 pt-12 border-b">
              <img src="/img/invoice.png" class="w-40 absolute -top-20" alt="logo-invoice" />
              <CardTitle class="text-4xl font-bold mt-12">
                {{ formatBaht(selectedItem?.amount ?? 0) }} บาท
              </CardTitle>
              <CardDescription>No. {{ selectedItem?.invoiceNo ?? "-" }}</CardDescription>
            </CardHeader>
            ...
            <CardContent class="flex items-start justify-between">
              ...
              <div class="flex flex-col text-right items-end gap-3">
                ...
                <span class="text-sm text-gray-500">{{ selectedItem?.content ?? "-" }}</span>
              </div>
            </CardContent>
          </BaseCard>
        </DrawerDescription>
      ...
      <div class="px-6 pb-6 space-y-3">
        <div class="flex justify-between">
          <span>จำนวนเงิน</span>
          <span class="font-medium">฿{{ selectedItem ? formatBaht(selectedItem.amount) : "" }}</span>
        </div>
      </div>
...
const accordionItems: AccordionItem[] = [
  {
    ...
    content: formatThaiDate("2025-08-01"),
    amount: 100000,
    status: "ส่งแล้ว",
  ...
  },
  {
    ...
    content: formatThaiDate("2025-12-31"),
    amount: 250000,
    status: "ยังไม่ส่ง",
  ...
  },
  {
    ...
    content: formatThaiDate("2025-09-15"),
    amount: 50000,
    status: "ยกเลิก",
  ...
  },
  {
    value: "item-4",
    title: "วันทดสอบระบบ",
    content: formatThaiDate("2025-10-20"),
    amount: 75000,
    status: "ยังไม่ส่ง",
  ...
  },
  {
    value: "item-5",
    title: "วันเปิดตัว",
    content: formatThaiDate("2026-01-05"),
    amount: 500000,
    status: "ส่งแล้ว",
  ...
  },
  {
    ...
    content: formatThaiDate("2026-12-31"),
    amount: 1200000,
    status: "ส่งแล้ว",
